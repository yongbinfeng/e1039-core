...
PHPythia8::PHPythia8(const std::string &name)
    : SubsysReco(name), _eventcount(0), _registeredTriggers(), _triggersOR(true), _triggersAND(false), _pythia(NULL), _configFile("phpythia8.cfg"), _commands(), _pythiaToHepMC(NULL), _save_integrated_luminosity(true), _integral_node(nullptr), _legacy_vertexgenerator(false)
{
...
{
  if (verbosity >= VERBOSITY_MORE)
    cout << "PHPythia8::process_event - event: " << _eventcount << endl;
  ...
  if (_eventcount < 2 && verbosity >= VERBOSITY_SOME)
    _pythia->event.list();
  if (_eventcount >= 2 && verbosity >= VERBOSITY_A_LOT)
    _pythia->event.list();
