...
VertexFit::VertexFit(const std::string &name) : SubsysReco(name)
{
  initGlobalVariables();
...
int VertexFit::InitField(PHCompositeNode *topNode)
{
  ...
  {
    if (verbosity > 1)
      cout << "VertexFit::InitField - create magnetic field setup" << endl;
...
int VertexFit::setRecEvent(SRecEvent *recEvent, int sign1, int sign2)
{
  ...
  {
    if (!recEvent->getTrack(idx_pos[i]).isValid())
      continue;
    if (Verbosity() > Fun4AllBase::VERBOSITY_A_LOT)
    {
    ...
    for (; j < nNeg; ++j)
    {
      ...
      if (fit_target_center)
        z_vertex_opt = Z_TARGET; // fit in the target center
  ...
  if (recEvent->getNDimuons() > 0)
    return VFEXIT_SUCCESS;
...
int VertexFit::findVertex()
{
  ...
  for (; nIter < _max_iteration; ++nIter)
  {
    ...
    if (m_displaced)
    {
      if (nIter > 0 && (fabs(_vtxpar_curr._r[2][0] - _node_vertex.getZ()) < _tolerance || _chisq_vertex > _chisq_kalman_prev))
        break;
      _chisq_kalman_prev = _chisq_vertex;
    }
    else
    {
      if (nIter > 0 && (fabs(_vtxpar_curr._r[2][0] - _node_vertex.getZ()) < _tolerance || _chisq_kalman > _chisq_kalman_prev))
        break;
  ...
  for (unsigned int it = 0; it < nIter; ++it)
  {
    ...
    if (m_displaced)
    {
      if (it > 0 && (fabs(_vtxpar_curr._r[2][0] - _node_vertex.getZ()) < _tolerance || _chisq_vertex > _chisq_kalman_prev))
        break;
      _chisq_kalman_prev = _chisq_vertex;
    }
    else
    {
      if (it > 0 && (fabs(_vtxpar_curr._r[2][0] - _node_vertex.getZ()) < _tolerance || _chisq_kalman > _chisq_kalman_prev))
        break;
      _chisq_kalman_prev = _chisq_kalman;
    }
  }
