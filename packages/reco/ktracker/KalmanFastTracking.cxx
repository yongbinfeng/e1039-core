...
bool KalmanFastTracking::acceptEvent(SRawEvent *rawEvent)
{
  ...
  if (rawEvent->getNHitsInD0() > MaxHitsDC0)
    return false;
  if (rawEvent->getNHitsInD1() > MaxHitsDC1)
    return false;
  ...
  for (std::list<Tracklet>::iterator tracklet3 = trackletsInSt[2].begin(); tracklet3 != trackletsInSt[2].end(); ++tracklet3)
  {
    ...
    for (std::list<Tracklet>::iterator tracklet2 = trackletsInSt[1].begin(); tracklet2 != trackletsInSt[1].end(); ++tracklet2)
    {
      if (!COARSE_MODE)
      {
        ...
        if (fabs(a) > 2. * TX_MAX || fabs(b) > 2. * X0_MAX)
          continue;
...
bool KalmanFastTracking::acceptTracklet(Tracklet &tracklet)
{
  ...
  if (tracklet.stationID > nStations - 2)
  {
    if (!COSMIC_MODE && !p_geomSvc->isInKMAG(tracklet.getExpPositionX(Z_KMAG_BEND), tracklet.getExpPositionY(Z_KMAG_BEND)))
      return false;
    if (!(muonID_comp(tracklet) || muonID_search(tracklet)))
      return false;
  }
...
int KalmanFastTracking::fitTracklet(Tracklet &tracklet)
{
  ...
  if (tracklet.err_tx < 1e-6)
    tracklet.err_tx = 1e-6;
  if (tracklet.err_ty < 1e-6)
    tracklet.err_ty = 1e-6;
  if (tracklet.err_x0 < 1e-4)
    tracklet.err_x0 = 1e-4;
  if (tracklet.err_y0 < 1e-4)
    tracklet.err_y0 = 1e-4;
