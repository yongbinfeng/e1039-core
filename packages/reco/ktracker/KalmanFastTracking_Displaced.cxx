...
int KalmanFastTracking_Displaced::setRawEventPrep(SRawEvent* event_input)
{
	...
	/* //Removed for displaced tracking, as x and y hodoscopes are both used
...
int KalmanFastTracking_Displaced::setRawEvent(SRawEvent* event_input)
{
  ...
  totalTime = 0;
    ...
    if(numHodoValidUXCombos > 10000 && !highPU){
      trackletsInSt23SlimX.clear();
      trackletsInSt23SlimU.clear();
      cutAdjuster(numHodoValidUXCombos, 1); //tighten matching requirements if there are too many potential combinations (normally the result of high pileup)
      adjusted = true;
      buildBackPartialTracksSlimX(reqHits, slopeComp, windowSize);
      buildBackPartialTracksSlimU(reqHits, slopeComp, windowSize);
    ...
    //Now make the actual station 2+3 tracklts
    ...
    //Check station 2+3 tracklet quality
	...
	checkSigns((*tracklet23)); //see if flipping the "side" of a wire that hits are on can improve the quality
    ...
    reduceTrackletList(trackletsInSt[3]); //remove redundant station 2+3 tracklets
    ...
    //Do quality control on global tracklets
...
void KalmanFastTracking_Displaced::buildBackPartialTracksSlim_v3(int cut)
{
  ...
  for(unsigned int tx = 0; tx < trackletsInSt23SlimX.size(); tx++){
    ...
    for(unsigned int com = 0; com < trackletsInSt23SlimX.at(tx).allowedVUXCombos.size(); com++){
      ...
      if( std::abs( (trackletX.st2X - trackletU.st2U) - -1.*(trackletX.st2X - trackletV.st2V) ) > XUVPosWin ) continue;
      if( std::abs( (trackletX.st3X - trackletU.st3U) - -1.*(trackletX.st3X - trackletV.st3V) ) > XUVPosWin ) continue;
	...
	if(tracklet_best.chisq < st23ChiSqCut){
...
void KalmanFastTracking_Displaced::buildBackPartialTracksSlimX(int pass, double slopeComparison, double windowSize)
{
  ...
  for(std::list<Tracklet>::iterator tracklet3 = trackletsInStSlimX[2][0][0].begin(); tracklet3 != trackletsInStSlimX[2][0][0].end(); ++tracklet3)
    {
	  ...
	  if(OLD_TRACKING){
	    ...
	    if(compareTrackletsSlim(*tracklet2, *tracklet3, pass, slopeComparison, windowSize) || (pass == 1 && compareTrackletsSlim_3hits(*tracklet2, *tracklet3, pass, slopeComparison, windowSize)) ){
	      ...
	      tracklet_23.x0 = tracklet2->st2Xsl * (0. - tracklet2->st2Z) + tracklet2->st2X; //simple extrapolation back to z = 0
	      tracklet_23.ty = 0.;
	      tracklet_23.y0 = 0.;
...
void KalmanFastTracking_Displaced::buildBackPartialTracksSlimU(int pass, double slopeComparison, double windowSize)
{
...
void KalmanFastTracking_Displaced::buildBackPartialTracksSlimV(int pass, double slopeComparison, double windowSize)
{
...
void KalmanFastTracking_Displaced::buildGlobalTracksDisplaced()
{
  ...
  tracklet23->print();
	  ...
	  for(int pxs = 0; pxs < 23; pxs++){
	    
	    if(validTrackFound && (pxs - validTrackPX) > 4) continue;
	  ...
	  if(tracklet_best_prob.chisq < 50. && tracklet_best_prob.isValid()) break;
	}
...
bool KalmanFastTracking_Displaced::buildTrackletsInStation1_NEW(int stationID, int listID, double expXZSlope, double expYSlope, double y0, bool tight, double* pos_exp, double* window)
{
#ifdef _DEBUG_ON
  LogInfo("Building tracklets in station " << stationID);
#endif
  ...
  //actual ID of the tracklet lists
  int sID = stationID - 1;
...
void KalmanFastTracking_Displaced::buildTrackletsInStationSlim(int stationID, int listID, double* pos_exp, double* window)
{
#ifdef _DEBUG_ON
...
void KalmanFastTracking_Displaced::buildTrackletsInStationSlimU(int stationID, int listID, double* pos_exp, double* window)
{
#ifdef _DEBUG_ON
...
void KalmanFastTracking_Displaced::buildTrackletsInStationSlimV(int stationID, int listID, double* pos_exp, double* window)
{
#ifdef _DEBUG_ON
...
bool KalmanFastTracking_Displaced::hodoMask(Tracklet& tracklet)
{
  ...
  if(TRACK_ELECTRONS && (tracklet.stationID == 4 || tracklet.stationID == 5)) return true; //Skip of hodoscope checks for station 3 tracks in the electron-tracking setup.  Could actually probably extrapolate backwards the station 2 hodoscope, now that I get an accurate X-Z slope in station 3
  int nHodoHits = 0;
  if(!OLD_TRACKING){
...
bool KalmanFastTracking_Displaced::compareTrackletsSlim(Tracklet& tracklet2, Tracklet& tracklet3, int pass, double slopeComparison, double windowSize)
{
...
bool KalmanFastTracking_Displaced::compareTrackletsSlim_3hits(Tracklet& tracklet2, Tracklet& tracklet3, int pass, double slopeComparison, double windowSize)
{
...
bool KalmanFastTracking_Displaced::compareTrackletsSlimU(Tracklet& tracklet2, Tracklet& tracklet3, int pass, double slopeComparison, double windowSize)
{
  if( !( tracklet2.hits.size() == 2 && tracklet3.hits.size() == 2 ) ) return false;
...
bool KalmanFastTracking_Displaced::compareTrackletsSlimU_3hits(Tracklet& tracklet2, Tracklet& tracklet3, int pass, double slopeComparison, double windowSize)
{
...
bool KalmanFastTracking_Displaced::compareTrackletsSlimV(Tracklet& tracklet2, Tracklet& tracklet3, int pass, double slopeComparison, double windowSize)
{
  if( !( tracklet2.hits.size() == 2 && tracklet3.hits.size() == 2 ) ) return false;
...
bool KalmanFastTracking_Displaced::compareTrackletsSlimV_3hits(Tracklet& tracklet2, Tracklet& tracklet3, int pass, double slopeComparison, double windowSize)
{
