...
int PropSegment::getNPlanes() const
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        if (hits[i].hit.index >= 0 && hits[i + 1].hit.index >= 0)
            ++nPlanes;
    }
    return nPlanes;
}
...
Tracklet::Tracklet() : stationID(-1), chisq(9999.), chisq_vtx(9999.), tx(0.), ty(0.), x0(0.), y0(0.), invP(0.1), err_tx(-1.), err_ty(-1.), err_x0(-1.), err_y0(-1.), err_invP(-1.), _charge(0) /// XinL: Maybe unproper if we need to track neutrual particles in the future
{
...
int Tracklet::isValid()
{
    ...
    int nAllHits = nHits[0] + nHits[1] + nHits[2];
    if (stationID < nStations - 1)
    {
        if (nHits[0] < 1 || nHits[1] < 1 || nHits[2] < 1)
            return 0;
        if (nAllHits < 4)
            return 0;
        if (chisq > 40.)
...
double Tracklet::getExpPositionW(int detectorID) const
{
    ...
    if (!p_geomSvc->isInPlane(detectorID, x_exp, y_exp))
        return 999999.;
...
{
    // return nHits[0] + nHits[1] + nHits[2] - 0.4*chisq > elem.nHits[0] + elem.nHits[1] + elem.nHits[2] - 0.4*elem.chisq;
...
bool Tracklet::similarity(const Tracklet &elem) const
{
    ...
    if (nCommonHits / double(elem.getNHits()) > 0.33333)
        return true;
    return false;
}
...
double Tracklet::similarity_st1(const Tracklet &elem) const
{
    ...
    while (first != hits.end() && second != elem.hits.end())
    {
        if ((*first).hit.detectorID < 7)
            n1st1++;
        if ((*second).hit.detectorID < 7)
            n2st1++;
        if ((*first) < (*second))
        {
            ++first;
...
bool Tracklet::similarityAllowed(const Tracklet &elem) const
{
    ...
    if (wiresMatch.at(0) && wiresMatch.at(1))
        allowed = false;
    if (wiresMatch.at(2) && wiresMatch.at(3))
        allowed = false;
    if (wiresMatch.at(4) && wiresMatch.at(5))
        allowed = false;
    if (double(double(nCommonHits) / double(n1st1)) > .17 || double(double(nCommonHits) / double(n2st1)) > .17)
        allowed = false;
...
{
    return _charge ? _charge : (-3e-3 * copysign(1.0, FMAGSTR) * x0 < tx ? +1 : -1);
}
...
Tracklet Tracklet::operator+(const Tracklet &elem)
{
    Tracklet tracklet;
    tracklet.stationID = nStations - 1;

    tracklet.nHits[0] = nHits[0] + elem.nHits[0];
    tracklet.nHits[1] = nHits[1] + elem.nHits[1];
    tracklet.nHits[2] = nHits[2] + elem.nHits[2];
...
void Tracklet::addDummyHits()
{
    ...
    for (int i = stationID * 6 - 5; i <= stationID * 6; ++i)
        detectorIDs_all.push_back(i);
...
void Tracklet::getSlopes(Hit hit1, Hit hit2, int plane_type)
{

    double zDist = p_geomSvc->getPlanePosition(hit2.detectorID) - p_geomSvc->getPlanePosition(hit1.detectorID);
    linedef testline1;
    testline1.slope = ((hit2.pos + hit2.driftDistance) - (hit1.pos + hit1.driftDistance)) / zDist;
    testline1.initial_pos = hit1.pos + hit1.driftDistance;
    testline1.initialZ = p_geomSvc->getPlanePosition(hit1.detectorID);

    linedef testline2;
    testline2.slope = ((hit2.pos + hit2.driftDistance) - (hit1.pos - hit1.driftDistance)) / zDist;
    testline2.initial_pos = hit1.pos - hit1.driftDistance;
    testline2.initialZ = p_geomSvc->getPlanePosition(hit1.detectorID);

    linedef testline3;
    testline3.slope = ((hit2.pos - hit2.driftDistance) - (hit1.pos + hit1.driftDistance)) / zDist;
    testline3.initial_pos = hit1.pos + hit1.driftDistance;
    testline3.initialZ = p_geomSvc->getPlanePosition(hit1.detectorID);

    linedef testline4;
    testline4.slope = ((hit2.pos - hit2.driftDistance) - (hit1.pos - hit1.driftDistance)) / zDist;
    testline4.initial_pos = hit1.pos - hit1.driftDistance;
    testline4.initialZ = p_geomSvc->getPlanePosition(hit1.detectorID);

    // Keep track of the four possible U-Z lines and the corresponding possible lines for the hit in the X-Y plane
    possibleLines.push_back(testline1);
    possibleLines.push_back(testline2);
    possibleLines.push_back(testline3);
    possibleLines.push_back(testline4);
}
...
void Tracklet::print(std::ostream &os)
{
    ...
    os << nHits[0] + nHits[1] + nHits[2] << " hits in this station with chisq = " << chisq << endl;
...
SignedHit Tracklet::getHit(int _i)
{
...
{
    for (auto iter = trackletVec.begin(); iter != trackletVec.end(); ++iter)
        delete (*iter);
    trackletVec.clear();
}
