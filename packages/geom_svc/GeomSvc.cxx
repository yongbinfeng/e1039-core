...
void GeomSvc::initPlaneDbSvc()
{
    ...
    for (int ii = 0; ii < geom->GetNumPlanes(); ii++)
    {
        ...
        if (planes[detectorID].x1 > x1_i)
            planes[detectorID].x1 = x1_i;
        if (planes[detectorID].x2 < x2_i)
            planes[detectorID].x2 = x2_i;
        if (planes[detectorID].y1 > y1_i)
            planes[detectorID].y1 = y1_i;
        if (planes[detectorID].y2 < y2_i)
            planes[detectorID].y2 = y2_i;
...
{
    if (x < planes[detectorID].x1 || x > planes[detectorID].x2)
        return false;
    if (y < planes[detectorID].y1 || y > planes[detectorID].y2)
        return false;
...
{
    if (x < xmin_kmag || x > xmax_kmag)
        return false;
    if (y < ymin_kmag || y > ymax_kmag)
        return false;
...
int GeomSvc::getExpElementID(int detectorID, double pos_exp)
{
    ...
    if (pos_exp > pos_max)
        return planes[detectorID].nElements + 1;
    if (pos_exp < pos_min)
        return 0;
    ...
    if (detectorID > nChamberPlanes + nHodoPlanes + nPropPlanes)
    {
        ...
        if (bottom)
            elementID = planes[detectorID].nElements + 1 - elementID;
    }
...
{
    if (detectorName.size() == 0 || detectorName[0] != 'H')
        return 0;
...
void GeomSvc::loadCalibration(const std::string &calibrationFile)
{
    ...
    if (_cali_file)
    {
        ...
        while (_cali_file.getline(buf, 100))
        {
            ...
            if (planes[detectorID].rtprofile != NULL)
...
void GeomSvc::printWirePosition()
{
    for (std::map<std::string, int>::iterator iter = map_detectorID.begin(); iter != map_detectorID.end(); ++iter)
    {
        if (iter->second > nChamberPlanes + nHodoPlanes + nPropPlanes)
            continue;
