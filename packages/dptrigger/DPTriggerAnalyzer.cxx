...
bool DPTriggerRoad::operator==(const DPTriggerRoad &elem) const
{
    for (unsigned int i = 0; i < NTRPLANES; ++i)
    {
        if (uniqueTrIDs[i] != elem.uniqueTrIDs[i])
            return false;
    }
...
DPTriggerAnalyzer::DPTriggerAnalyzer(const std::string &name) : SubsysReco(name),
...
int DPTriggerAnalyzer::process_event(PHCompositeNode *topNode)
{
    ...
    {
        if (_req_intime && !_hit_vector->at(ihit)->is_in_time())
            continue;
...
int DPTriggerAnalyzer::GetNodes(PHCompositeNode *topNode)
{
    ...
    { // Create and use a new node for output
