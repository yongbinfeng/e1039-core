...
TruthNodeMaker::TruthNodeMaker()
    : SubsysReco("TruthNodeMaker"), genevtmap(nullptr), g4true(nullptr), m_vec_hit(nullptr), m_rec_evt(nullptr), m_vec_rec_trk(nullptr), m_evt(nullptr), m_mcevt(nullptr), m_vec_trk(nullptr), m_vec_dim(nullptr), m_job_id(-1), m_legacy_rec_container(true), m_do_evt_header(true), m_do_truthtrk_tagging(true), m_matching_threshold(0.75), m_process_type(4)
{
...
int TruthNodeMaker::process_event(PHCompositeNode *topNode)
{
  ...
  for (unsigned int i1 = 0; i1 < n_trk; i1++)
  {
    ...
    if (trk1->get_charge() <= 0)
      continue;
    for (unsigned int i2 = 0; i2 < n_trk; i2++)
    {
      SQTrack *trk2 = m_vec_trk->at(i2);
      if (trk2->get_charge() >= 0)
        continue;
      if (vec_vtx_id[i1] != vec_vtx_id[i2])
        continue;
  ...
  for (int i = 0; i < n_digihits; ++i)
  {
    ...
    if (detid > nChamberPlanes || (detid >= 7 && detid <= 12))
      continue;

    int trkid = hit->get_track_id();
    if (trkid_idx.find(trkid) == trkid_idx.end())
      continue;
