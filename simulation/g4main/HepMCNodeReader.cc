...
HepMCNodeReader::HepMCNodeReader(const std::string &name)
	: SubsysReco(name), use_seed(0), seed(0), vertex_pos_x(0.0), vertex_pos_y(0.0), vertex_pos_z(0.0), vertex_t0(0.0), width_vx(0.0), width_vy(0.0), width_vz(0.0), _bkg_mode(false) // Abi
	  ,
	  _pxy2pz_rat(0.25), _particle_filter_on(false), _position_filter_on(false), _pos_filter_x_min(0.0), _pos_filter_x_max(0.0), _pos_filter_y_min(0.0), _pos_filter_y_max(0.0), _pos_filter_z_min(0.0), _pos_filter_z_max(0.0)
{
			...
			if (!finalstateparticles.empty())
			{
				...
				if (_position_filter_on &&
					(xpos < _pos_filter_x_min || xpos > _pos_filter_x_max ||
					 ...
					 zpos < _pos_filter_z_min || zpos > _pos_filter_z_max))
					continue;
					 ...
					 ++fiter)
				{
					...
					if (_bkg_mode)
					{ //@ Filters for the background candidates
						...
						if (fabs((*fiter)->momentum().px() * mom_factor / ((*fiter)->momentum().pz() * mom_factor)) > _pxy2pz_rat)
							continue;
						if (fabs((*fiter)->momentum().py() * mom_factor / ((*fiter)->momentum().pz() * mom_factor)) > _pxy2pz_rat)
							continue;
						if ((*fiter)->momentum().pz() < 0)
							continue;

						/// Fmag fucidual cut
						if (fabs(xpos) > FMAG_X / 2.)
							continue;
						if (fabs(ypos) > FMAG_Y / 2.)
							continue;
						...
						if (genevt->get_collision_vertex().z() > -664. && genevt->get_collision_vertex().z() < -542. && (fabs(xpos) > coll_x / 2. || fabs(ypos) > coll_y / 2.))
						{
							if (zpos < -542.)
							{
								if (gRandom->Uniform(0, 1) > exp(-(-genevt->get_collision_vertex().z() + zpos) / (pion_lambda_Cu)))
									continue;
							}
							else
							{
								if (gRandom->Uniform(0, 1) > exp(-(-genevt->get_collision_vertex().z() - 542.) / (pion_lambda_Cu)))
									continue;
							}
						}
						...
						if (genevt->get_collision_vertex().z() > -171.5 && genevt->get_collision_vertex().z() < -0.001)
						{
							if (zpos < -0.001)
							{
								if (gRandom->Uniform(0, 1) > exp(-(-genevt->get_collision_vertex().z() + zpos) / pion_lambda_sh))
									continue;
							}
							else
							{
								if (gRandom->Uniform(0, 1) > exp(-(171.5) / pion_lambda_sh))
									continue;
							}
						}
						...
						if (genevt->get_collision_vertex().z() < -171.5 && Transverse_pos < 7.62)
						{
							...
							{
								if (gRandom->Uniform(0, 1) > exp(-(171.5 + zpos) / pion_lambda_sh))
									continue;
							}
							if (zpos > -.001)
							{
						...
						{
							if (gRandom->Uniform(0, 1) > exp(-(zpos - genevt->get_collision_vertex().z()) / pion_lambda_Fe))
								continue; // decay length cut
						}
						...
						if (genevt->get_collision_vertex().z() < 0. && zpos > 0.)
						{
							...
							{
								if (gRandom->Uniform(0, 1) > exp(-zpos / pion_lambda_Fe))
									continue;
							}
							else
							{
								if (gRandom->Uniform(0, 1) > exp(-(zpos - FMAG_hole_L) / pion_lambda_Fe))
									continue;
							}
						}

						// Filter to get decay muons having enough energy to pass trough FMAG
						if (zpos < 0. && totMom / (FMAG_Z) < 0.01)
							continue;
						if (zpos > 0. && totMom / (FMAG_Z - zpos) < 0.01)
							continue;
					} //@
